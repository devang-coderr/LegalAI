...
import { CheckCircle2, Clock3, FileCheck, ShieldCheck, Upload, XCircle } from "lucide-react";
...
export default function LawyerVerificationPage() {
  ...
  useEffect(() => { setUser(getSession()); try { const raw=localStorage.getItem("legalai-verification-documents"); if(raw) setDocs(JSON.parse(raw)); } catch {} }, []);
  ...
  return <div className="space-y-8 py-4">
    <div className="space-y-2"><Badge variant={status === "VERIFIED" ? "default" : "warning"}>{status === "VERIFIED" ? "Identity verified" : "Verification required"}</Badge><h1 className="font-serif text-4xl font-bold">Lawyer verification</h1><p className="max-w-2xl text-sm leading-6 text-[var(--text-secondary)]">LegalAI separates professional access from ordinary registration. In this frontend-only prototype, documents are collected for review and the final decision is represented by the verification workflow.</p></div>
    <div className="grid gap-6 lg:grid-cols-[1.35fr_.65fr]">
      ...
      <Card variant="glass" className="h-fit space-y-5 p-6"><h2 className="font-serif text-xl font-bold">Verification actions</h2><p className="text-xs leading-5 text-[var(--text-secondary)]">Frontend demo controls let your SIH team demonstrate both sides of the workflow without claiming that a real-world licence was verified.</p><div className="space-y-3"><div className="flex items-center gap-3 rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] p-3"><Upload className="h-4 w-4 text-[var(--accent-blue)]"/><span className="text-xs">Documents submitted at signup</span></div><div className="flex items-center gap-3 rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] p-3"><FileCheck className="h-4 w-4 text-[var(--accent-gold)]"/><span className="text-xs">Enrollment number captured</span></div></div>{status !== "VERIFIED" ? <Button onClick={simulateVerification} className="w-full">Simulate admin verification</Button> : <Link href="/lawyer" className="block"><Button className="w-full">Enter Lawyer Workspace</Button></Link>}<Link href="/lawyer/settings" className="block text-center text-xs text-[var(--accent-blue)]">Open verification settings</Link></Card>
    </div>
  </div>;
}
